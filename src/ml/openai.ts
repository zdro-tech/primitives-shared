...
export const newGPT54Completion = async (
  messages: ChatCompletionMessageParam[],
  mode?: string
): Promise<ChatCompletion.Choice[]> =>
  await createChatCompletion({ model: "gpt-5.4", messages, max_completion_tokens: 8192 }, mode);

export const newGPT54MiniCompletion = async (
  messages: ChatCompletionMessageParam[],
  mode?: string
): Promise<ChatCompletion.Choice[]> =>
  await createChatCompletion({ model: "gpt-5.4-mini", messages, max_completion_tokens: 8192 }, mode);

export const newGPT54NanoCompletion = async (
  messages: ChatCompletionMessageParam[],
  mode?: string
): Promise<ChatCompletion.Choice[]> =>
  await createChatCompletion({ model: "gpt-5.4-nano", messages, max_completion_tokens: 8192 }, mode);
